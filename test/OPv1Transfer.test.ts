...
describe("OPv1Transfer", () => {
  ...
  beforeEach(async () => {
    const OPv1Transfer = await ethers.getContractFactory(
      "OPv1TransferTestInterface",
    );
    transfer = await OPv1Transfer.deploy();
  ...
  describe("transferFromAccount", () => {
    ...
    it("reverts when mistakenly trying to transfer Ether", async () => {
      for (const balance of [
        ...
        ).to.be.revertedWith("OPv1: cannot transfer native ETH");
      }
    });
