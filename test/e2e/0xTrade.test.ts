...
describe("E2E: Can settle a 0x trade", () => {
  ...
  function generateSettlementSolution(): {
    ...
    const zeroExTakerAmount = opv1Order.buyAmount.mul(zeroExGnoPrice);

    const opv1GnoPrice = 120;
    const clearingPrices = {
      [owl.address]: 1,
      [gno.address]: opv1GnoPrice,
    };

    const opv1OwlSurplus = opv1Order.sellAmount.sub(
      opv1Order.buyAmount.mul(opv1GnoPrice),
    );
    const zeroExOwlSurplus = opv1Order.buyAmount.mul(
      opv1GnoPrice - zeroExGnoPrice,
    );
  ...
  describe("0x Protocol v2", () => {
    it("should settle an EOA trade with a 0x trade", async () => {
      ...
      await encoder.signEncodeTrade(opv1Order, trader, SigningScheme.EIP712);
