...
contract TestTransferOwnership is Test {
    ...
    function test_reads_address_json_on_existing_chain_id() public {
        uint256 chainId = 1;
        assertEq(
            networksJson.addressByChainId("OPv1Settlement", chainId),
            address(0x9008D19f58AAbD9eD0D60971565AA8510560ab41)
        );
        vm.chainId(chainId);
        assertEq(networksJson.addressOf("OPv1Settlement"), address(0x9008D19f58AAbD9eD0D60971565AA8510560ab41));
    }
    ...
    function test_reverts_reads_address_json_on_unsupported_chain_id() public {
        ...
        vm.chainId(chainId);
        vm.expectRevert();
        networksJson.addressOf("OPv1Settlement");
    }
}
