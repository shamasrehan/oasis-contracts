...
describe("OPv1AllowListAuthentication", () => {
  ...
  beforeEach(async () => {
    const OPv1AllowListAuthentication = await ethers.getContractFactory(
      "OPv1AllowListAuthenticationTestInterface",
      deployer,
    );
    ...
    authenticator = await OPv1AllowListAuthentication.deploy(owner.address);
  ...
  describe("addSolver(address)", () => {
    ...
    it("should not allow owner to add solver", async () => {
      ...
      ).to.be.revertedWith("OPv1: caller not manager");
    });
